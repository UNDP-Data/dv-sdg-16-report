...
function useData() {
  return useQuery({
    queryKey: ['representation-by-institutional-level'],
    queryFn: () =>
      fetchAndParseJSON(
        '/data/report/04-inclusion/16-7-1-b-c/representation-by-institutional-level.json',
      ) as Promise<DataType[]>,
  });
}
  ...
  return (
    <div className='flex flex-col gap-4' style={{ padding: CHART_PADDING }}>
      ...
      <SimpleBarGraph
        ...
        labelOrder={data.filter((d) => d.sector === selectedSector).map((d) => d.level)}
